...
Movement/cavalry phases send every army straight at the nearest enemy
outpost to destroy it (2 VP for the kill, plus it denies that opponent
their per-round outpost VP going forward) - or, if no enemy outpost
exists anywhere yet, at the nearest enemy capital instead, just to keep
units advancing rather than sitting idle.
...
def greedy_rush_move(state, faction, legal_mask):
    ...
    that most reduces hex distance to the nearest enemy outpost - or,
    if none exist yet, the nearest enemy capital. No threat-awareness:
    always takes the closest-approaching legal step regardless of
    what's in the way, mirroring smart_random_agent's
    greedy_move_largest_army but targeting outposts specifically."""
    grid = state.grid
    ...
    for origin in ranked:
        target = min(targets, key=lambda c: hex_distance(grid.coord_of(origin), c))
