...
int main(int argc, char** argv) {
    ...
    for (int g = 0; g < num_games; ++g) {
        // Rotating shifts the assignment one seat per game, so each kind samples
        // every seat roughly evenly - placement/draft order is not perfectly
        // symmetric across seats, and this cancels that out.
        ...
        const int64_t seed = base_seed + g;
        oo::AgentSet agents;
        oo::build_mixed_agents(agents, this_game.data(), num_factions, seed);
        const oo::GameResult r = oo::play_game(agents, radius, num_factions, seed, max_turns);

        total_turns += r.turns;
        if (r.winner < 0) ++no_winner;
        for (int f = 0; f < num_factions; ++f) {
            const int k = static_cast<int>(this_game[static_cast<size_t>(f)]);
            ++kind_games[k];
