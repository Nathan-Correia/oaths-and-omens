...
resources (gold, kill_xp, alive) and turn_number together make up the
...
class ArrayState:
    ...
    outpost_upgrade: np.ndarray  # int8[N]             - NO_UPGRADE, or an index into UPGRADE_TYPES;
    ...
    gold: np.ndarray             # int32[num_factions]
    resources: np.ndarray        # int32[num_factions, 4] - Wood/Iron/Clay/Fish, see RESOURCE_TO_INDEX
...
def new_empty(grid, num_factions):
    ...
    return ArrayState(
        ...
        outpost_upgrade=np.full(n, NO_UPGRADE, dtype=np.int8),
        ...
        battle_order=[],
        gold=np.zeros(num_factions, dtype=np.int32),
        resources=np.zeros((num_factions, len(RESOURCE_TYPES)), dtype=np.int32),
